...
public partial class Areas_Client_SanPham_SanPham_DanhSach : System.Web.UI.UserControl
{
    string id = "";
    string SoSPHienThi = "";
    string _p = "1";
    ...
    private string LayMoTa()
    {
        ...
        {
            s += @"
     <div class='title'>" + dt.Rows[i]["TenDMSP"] + @"</div>
                <div class='decs'>
                    " + dt.Rows[i]["MoTa"] + @"
                </div>
    ";
...
";
            s += @"
            <div class='soluong'>(" + (LaySoLuong(dt.Rows[i]["MaDMSP"].ToString())) + @")</div>
    ";
            s += @"
...
";
    ...
    private string LayDanhSachSanPham(string id, string SoSPHienThi)
    {
        string s = "";
        var pageSize = "8"; // Số sản phẩm trên 1 trang
        var condition = "MaDMSP = " + id; // Điều kiện lấy sản phẩm, có thể lấy theo mã danh mục hoặc trạng thái on/off của sản phẩm
        var orderBy = ""; // Trường sắp xếp kết quả
        var ds = DongPhucKhanhLinh.SanPham.GetDataPaging(_p, pageSize, condition, orderBy);
        var dt = ds.Tables[0];
        //DataTable dt = new DataTable();
        //dt = DongPhucKhanhLinh.SanPham.thongtin_sanpham_by_dmid(id, SoSPHienThi);
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            s += @"
...
";
        ...
        ltrphantrang.Text += PagingExtensions.SpilitPagesNoRewrite(Convert.ToInt32(totalRows), Convert.ToInt32(pageSize), Convert.ToInt32(_p),
            "/default.aspx?dm=san-pham&danhmucphu=danhsach&id=" + id, "active", "normal", "first", "last", "preview", "next");
        return s;
    }
